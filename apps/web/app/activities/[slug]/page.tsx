...
export default function ActivityPage() {
  ...
  const now = Date.now();
  const accepting =
    now < new Date(activity.submission_ends_at).getTime();
  const voting =
    now >= new Date(activity.voting_starts_at).getTime() &&
    now < new Date(activity.voting_ends_at).getTime();
        ...
        <main className="content-main">
          <div className="page-header">
            ...
            <p className="page-header__description">
              投稿截止：
              {new Date(activity.submission_ends_at).toLocaleString("zh-CN")} ·
              可投 {activity.vote_limit} 票
            </p>
          </div>
            ...
            >
              ...
              <div className="form-group">
                ...
                <span className="form-hint">
                  最多上传 {IMAGE_RULES.maxSubmissionImages} 张图片
                </span>
              </div>
          ...
          <div className="content-section">
            ...
            <div className="post-feed">
                ...
                <div className="comment-card" key={entry.id}>
                  <div className="comment-card__header">
                    ...
                    <span className="comment-card__date">
                      {entry.profiles?.display_name || "成员"}
                    </span>
                  </div>
                  {voting && (
                    <button
              ...
              {entries.length === 0 && (
                <p className="text-muted">暂无已审核作品。</p>
              )}
            </div>
          </div>
